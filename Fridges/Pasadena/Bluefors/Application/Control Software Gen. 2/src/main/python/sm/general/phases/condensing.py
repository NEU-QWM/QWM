...
class Condensing(Procedure):
    ...
    def validate(self, parameters, state):
        ...
        if state["P5_PRESSURE"] < 0.015:
            yield ValidationError(1649, "Mixture already in circulation")

    def enter(self, parameters):
        logger.info('Entering Condensing')

        if state["P5_PRESSURE"] < (parameters["initialTankPressure"]*0.95):
            self.command_queue.set_state({"valves": ["V202","V204NO", "V205NO", "V206NO"], "pumps": []})
            self.command_queue.valve_commands = [c for c in self.command_queue.valve_commands if c.device_id != 'V201G']
        else:
            self.command_queue.set_state({"valves": ["V204NO", "V205NO", "V206NO"], "pumps": []})
        ...
        #     self.command_queue.queue_valves_off(['V104', 'V106', 'V107'])
        # else:
        #     self.command_queue.queue_valves_off(['V105', 'V114', 'V112'])
    ...
    def iterate_condensing_high_pressure_loop(self, parameters, heat_switch_trigger_pressure):
        ...
        while state["P4_PRESSURE"] < 0.900:
            ...
            if state["P5_PRESSURE"] < 0.5: # only triggers at the end of condensing
                if sw.elapsed > 120:
                    logger.info("High pressure condensing: Needle valve set too tight, check adjustment")
                    break
...
class IdleCirculating(OperationProcedure):
    ...
    '''
    Sequence:
    - Confirm that the tank is empty, T4K < 4.2 K and Tstill < 1.6 K
    '''
    ...
    def validate(self, parameters, state):
        if state["P5_PRESSURE"] > 0.030:
            yield ValidationError(1649, "Ready for condensing finalization")
        if state["4K_TEMPERATURE"] > 4.2 or state["STILL_TEMPERATURE"] > 1.6:
