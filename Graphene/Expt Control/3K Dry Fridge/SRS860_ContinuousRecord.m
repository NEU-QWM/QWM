...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function CoolLogData = SRS860_ContinuousRecord()
temp = instrfind;
if ~isempty(temp)
    fclose(temp)
    delete(temp)
...
SRS = deviceDrivers.SRS860();
SRS.connect('4');

% Initialize variables
DataInterval = input('Time interval between measurment (in second) = ');
FileNumber = input('file nnumber used as suffix = ','s');
start_dir = 'C:\Users\Columbus\Documents\PROJECTS\High-Q Resonator\Run01';
start_dir = uigetdir(start_dir);
StartTime = clock;
FileName = strcat('SRS860_', datestr(StartTime, 'yyyymmdd_HHMMSS'),FileNumber, '.dat');
FilePtr = fopen(fullfile(start_dir, FileName), 'w');
fprintf(FilePtr, strcat(datestr(StartTime), ' Lockin Values using SRS860\r\n'));
...
while true
    ...
    fclose(FilePtr);
    clf
    plot(SRSData(:,1)/60, SRSData(:,2)); grid on; xlabel('Time (min.)'); ylabel('X & Y (V)'); title(strcat('SRS860 Lockin Monitor, start date and time: ', datestr(StartTime)));
    hold all
