...
with program() as prog:
    ...
    with stream_processing():
        ...
        I_st.buffer(len(spec_sweep_dfs)).buffer(len(durations)).average().save("I")
        Q_st.buffer(len(spec_sweep_dfs)).buffer(len(durations)).average().save("Q")
        n_st.save("iteration")
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot results
        plt.subplot(211)
        plt.suptitle(f"{qubit_key}, Rabi chevron, ({iteration+1}/{n_avg}), amp {x180_amp}")
        plt.cla()
        plt.title(r"$R=\sqrt{I^2 + Q^2}$")
        plt.pcolor(spec_sweep_dfs / u.MHz, durations * 4, R)
        plt.ylabel("Pulse duration [ns]")
        plt.subplot(212)
        plt.cla()
        plt.title("Phase")
        plt.pcolor(spec_sweep_dfs / u.MHz, durations * 4, np.unwrap(phase))
