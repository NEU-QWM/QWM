...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"{qubit_key},Readout frequency optimization around {res_IF / u.MHz} MHz, {iteration+1}/{n_avg}")
        plt.xlabel("Readout frequency detuning [MHz]")
        plt.ylabel("SNR")
        plt.grid("on")
        plt.pause(0.1)
    plt.show()
    print(f"The optimal readout frequency is {dfs[np.argmax(SNR)] + res_IF} Hz (SNR={max(SNR)})")
    # Save results
