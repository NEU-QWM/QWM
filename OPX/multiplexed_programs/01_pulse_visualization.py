...
res_key, depletion_time = single_qubit_parser(multiplexed_parameters.copy(), qubit_key, call_list=required_parameters)
...
with program() as raw_trace_prog:
    reset_global_phase()
    t = declare(int)  # QUA variable for the averaging loop
    adc_st = declare_stream(adc_trace=True)  # The stream to store the raw ADC trace

    with for_(t, 0, t < 500, t + 32):
        # Reset the phase of the digital oscillator associated to the resonator element. Needed to average the cosine signal.
        reset_phase(res_key)
        # Sends the readout pulse and stores the raw ADC traces in the stream called "adc_st"
        # wait(mw_pulse_delay * u.ns, "r1")
        play("x180"*amp(1), "q1", duration = t)
        # measure("readout", res_key, adc_st)
        # Wait for the resonator to deplete
        # wait(depletion_time//4, res_key)
...
if simulate:
    ...
    save_data_dict.update({"adc_real": adc.real})
    save_data_dict.update({"adc_imag": adc.imag})
    save_data_dict.update({"fig": fig})
    data_handler.additional_files = {**default_additional_files}
    data_handler.save_data(data=save_data_dict, name="_".join(script_name.split("_")[1:]).split(".")[0])

    # Update the config
    print(f"Set time of flight: {time_of_flight + delay} ns")