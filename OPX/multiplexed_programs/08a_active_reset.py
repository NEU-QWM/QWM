...
qubit_key = "q1"
required_parameters = ["resonator_key", "readout_len", "resonator_relaxation", "qubit_relaxation", "ge_threshold"]
res_key, readout_len, res_relaxation, qubit_relaxation, ge_threshold = single_qubit_parser(multiplexed_parameters.copy(), qubit_key, call_list=required_parameters)
...
def active_reset_one_threshold(threshold_g: float, max_tries: int):
    ...
    with while_((I_reset > threshold_g) & (counter < max_tries)):
        ...
        measure("readout", res_key, dual_demod.full("opt_cos", "opt_sin", I_reset))
        align(res_key, qubit_key)
        # Wait for the resonator to deplete
        wait(depletion_time, qubit_key)
        # Play a conditional pi-pulse to actively reset the qubit
        play("x180", qubit_key, condition=(I_reset > threshold_g))
...
def active_reset_two_thresholds(threshold_g: float, threshold_e: float, max_tries: int):
    ...
    with while_((I_reset > threshold_g) & (counter < max_tries)):
        ...
        wait(depletion_time, qubit_key)
        # Play a conditional pi-pulse to actively reset the qubit
        play("x180", qubit_key, condition=(I_reset > threshold_e))
        # Update the counter for benchmarking purposes
        assign(counter, counter + 1)
    return counter
...
with program() as prog:
    ...
    with for_(n, 0, n < n_shot, n + 1):
        # Active reset
        counter = qubit_initialization(method=initialization_method)
        align()
