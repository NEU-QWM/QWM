...
n_avg = 32  # Number of averaging loops for each random sequence
max_circuit_depth = 40  # Maximum circuit depth
delta_clifford = 1  #  Play each sequence with a depth step equals to 'delta_clifford - Must be > 0
        ...
        with for_(depth, 1, depth <= max_circuit_depth, depth + 1):  # Loop over the depths
            ...
            with if_(depth == depth_target):
                with for_(n, 0, n < n_avg, n + 1):  # Averaging loop
                    ...
                    state, I, Q = readout_macro(res_key, threshold=ge_threshold, state=state, I=I, Q=Q)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"{qubit_key}, Single qubit RB ({iteration+1}/{num_of_sequences})")
        plt.pause(0.1)
    ...
    plt.xlabel("Number of Clifford gates")
    plt.ylabel("Sequence Fidelity")
    plt.title(f"{qubit_key}, Single qubit RB")
