...
def generate_sequence(interleaved_gate_index):
    ...
    sequence_single = declare(int, size=2 * 2 * max_circuit_depth + 1)
    ...
    recovery_pairs = declare(int, size=2 * 2 * max_circuit_depth)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"{qubit_key}, Interleaved RB 20ns [{get_interleaved_gate(interleaved_gate_index)}] ({iteration + 1}/{num_of_sequences})")
        plt.pause(0.1)
    ...
    plt.xlabel("Number of Clifford gates")
    plt.ylabel("Sequence Fidelity")
    plt.title(f"{qubit_key}, Interleaved RB 20ns [{get_interleaved_gate(interleaved_gate_index)}]")
